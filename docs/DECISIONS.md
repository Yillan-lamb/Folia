...
### 2026-06-05 17:35 (Codex)

- **目标:** 评估外部 PR #21 ~ #26，制定最小影响主功能的合并策略
- **结论:**
  1. #22 可以吸收“新建临时 Markdown 草稿可放弃并回到原文件”的产品意图，但不直接合原 PR；应在主线重做小补丁，补 `Cmd+N` 实际快捷键、未保存变更保护、打开 / 保存 / 放弃后的状态闭环和完整测试。
  2. #23 不合入原 PR。它用原生 `EditorView` 替换现有 `@uiw/react-codemirror` 封装，丢失或弱化撤销/重做、搜索、行号、拼写检查、自动换行开关等源码编辑核心能力，风险高于“同步 active document”的收益。
  3. #26 不按原 PR 合入。主线已有 `tocPinned` 与轨道点击固定/取消固定；原 PR 进一步把固定大纲改成左侧常驻栏，会改变阅读版面和右侧预览共存行为，不属于低风险补丁。
  4. #21、#24、#25 维持关闭：#21 引入 localhost 外部 WebView 和 Tauri 权限扩张并拦截关闭窗口；#24 是未完整闭环的双栏对比功能；#25 将 HTML 内置预设从 3 套扩到 10 套并放宽来源合规测试，回退近期减法决策。
- **验证:** 已读取 GitHub PR 元数据、变更文件和 diff；已拉取 `origin/pr-21` ~ `origin/pr-26` 并用 `git merge-tree --write-tree origin/main origin/pr-*` 检查当前远端 `main` 无文本冲突；`git diff --check origin/main...origin/pr-*` 均通过。未 checkout PR 分支运行测试，原因是当前工作区已有大量未提交改动，本轮仅做评估。
- **影响:** 后续若要合并贡献，优先建立干净分支，只摘取小的产品意图并按 `CONTRIBUTING.md` 要求补全验证；不把大范围布局、权限、编辑器内核或预设体系改动作为顺手合并内容。

### 2026-06-05 14:17 (Claude)

- **目标:** 补建项目 `.codex/` 目录，同步全局协议版本号引用，恢复项目级 Skill 在 Codex 模式下的可读性
- **操作:**
  1. 调用 `project-init/scripts/init.sh codex` 创建 `.codex/config.toml`、`.codex/rules/default.rules`
  2. 创建 `.codex/skills` 符号链接 → `../.claude/skills`，与 `.claude/skills/` 共享 6 个已安装 Skill（git-batch-commit、git-workflow、release-workflow、multi-agent-orchestration、cross-agent-coordination、agent-email）
  3. 修正 `AGENTS.md` 第 9 行全局协议版本号引用：`v3.5` → `v3.8`
- **结果:** Codex 模式可读取项目级 Skill，git-batch-commit 等 workflow 后续会自动生效
- **下一步:** 用 git-batch-commit 风格提交本次补建，并验证后续提交是否按预期切分

### 2026-06-05 16:35 (Claude)

- **目标:** 落地 ISS-139，给外部贡献者和 AI Agent 提供项目级贡献规范
- **操作:**
  1. 新增 `CONTRIBUTING.md`（约 170 行）：项目定位、必读文档、必跑验证、PR 流程、明确禁止项（破坏核心功能 / 危险桌面行为 / 回退项目决策 / 污染主分支 四类）、AI Agent 专项、失败/阻塞、行为准则、目录速查
  2. 新增 `.github/PULL_REQUEST_TEMPLATE.md`：PR 描述模板，强制 Summary / Why / Test Plan / Risk & Rollback / Checklist 五段
  3. `README.md` 在"构建"和"许可"之间加"贡献"段，链到 `CONTRIBUTING.md`
  4. `docs/TASKS.md` 新增 ISS-139 任务描述与完成日志
- **结果:** 单 commit `b15a623 docs: add contributing guide and PR template`，4 文件 / 322 行新增。提交消息含 `Refs ISS-139`
- **下一步:** 外部 PR 评审时按 §5 明确禁止项做门禁；新功能 PR 提交前对照 §3 必跑验证贴出实际命令输出
...
### [DEC-143] - 2026-09-06 - 固定大纲宽度可调节 + 「总是固定大纲」收敛到设置页（ISS-217）

**背景**：用户反馈两点：①固定态大纲左栏宽度固定 260px 不可调，长标题条目换行局促；②「总是固定大纲」开关按钮出现在大纲面板头部下方，与固定/关闭按钮并列显得拥挤难看。前者是能力缺口，后者触碰 DESIGN.md「Floating TOC」一节明确记载的「『总是固定大纲』只在固定态左侧栏中出现」设计——按 CONTRIBUTING §5.3 在本条与 PR 中显式记录。
...
**根因**（vendored `lute.min.js` node 实测证实）：CommonMark 规定非 `<>` 包裹的行内图片**目标地址不允许空格**——空格即目标结束，其后内容无法构成合法 title，整个 `![…](…)` 构造解析失败、按普通文本输出。Lute 严格遵循（无宽松开关，Vditor 内部自建 Lute 实例也无法注入自定义解析）。用户原文件 94 张图归一化前渲染 `<img>` 数为 **0**，`localImageResolver` 根本没有 `<img>` 可处理。

**决策**：读盘装载层做纯文本归一化，而不是改渲染层或 vendor 补丁。

1. **落点**：`fileService.openPath` markdown 分支。三条读盘路径（打开对话框 / ISS-188 外部修改自动重载 / 系统「打开方式」）在此唯一收口，单点接入全覆盖；HTML 文件不经处理（`htmlPresentationService` 自行内联资源）。
2. **形式**：未转义空格 / Tab → `%20` / `%09`（不是 `<…>` 包裹）。理由：Lute 自身序列化就用 `%20`（编辑器 IR 往返无二次 diff）；`%20` 在 GitHub / VS Code / cmark 等任何严格解析器下都合法，URL 解码后语义与原路径一致——相对原文 strictly better。
...
**已知边界**：引用式定义 `[ref]: ./x y.png` 的目标空格、普通链接目标空格不在本期范围（转录场景未出现，行为最小变更）；缩进式代码块（4 空格）不跳过——法律文档罕见且与列表内嵌图片互斥，接受；HTML 属性值 / HTML 块内的 `![…](…)` 字面文本会被归一化（HTML 上下文内 Markdown 本不解析、不影响渲染，仅改写字面文本，登记为已知边界，PR #131 review m-1）；目标含未转义空格**且** title 内含 `\"` 转义后跟 `)` 的非法构造，归一化时 title 会被并入目标一并编码——此类输入本就因目标空格渲染不出图片，无渲染回归，合法 title 转义形态实测全部正确（PR #131 review M-2，降级 MINOR）；`\r\r\n` 双 CR 行尾不识别为围栏闭合（无现实工具产出该形态）。
...
**决策/结论**：内置 6 套主题（浅色 / 羊皮纸 Sepia / 青纸 Sage / 深色 / 夜墨 Ink / 古典 Classic）+ 自定义 CSS 导入（复用 HTML 导出预设的「槽位 + license」机制，标准 2 个、内测码扩容到 8 个）；CSS 安全 sanitize（剥 `@import` / `url(javascript:/vbscript:/data:text/html)` / `expression(` / `-moz-binding` / `behavior:`）。作用范围严格限定阅读写作环境，导出面（公众号 HTML 预览 / Word 纸张 / HTML 演示 iframe）不消费主题变量，加 e2e 守卫防渗漏。`theme` 字段从 `'light'|'dark'` 升级为 `themeId: string`，旧值平滑迁移。另：设置页新增「设为默认 Markdown 应用」按钮（macOS 调 `LSSetDefaultRoleHandlerForContentType`，非 macOS 返回 `unsupported`）。

**验证**：typecheck 0 error / lint 0 error / npm test 709/709 / build 成功 / e2e theme-system-guards 2/2。**真机验证（NOT_VERIFIED，移交用户）**：WKWebView 内 6 套主题观感、自定义 CSS 导入、重启保留；「双击 .md 由 Folia 打开」端到端未真机确认。

### [DEC-136] - 2026-08-12 - 双击选词后首次 Backspace/Delete 失效（PR #113）

**背景**：Vditor IR 的 click handler 每次 click 把 selection 重置为 collapsed，覆盖浏览器原生「dblclick 选词」——视觉上词被选中，JS 选区却为空，Backspace 走光标前向删除首次无可见效果。

**决策/结论**：在 host 上挂 capture-phase dblclick 监听，用 `caretRangeFromPoint` 反推词边界重建 selection；不调 `preventDefault`。不改动 ISS-69（PR #73）的 `captureSelectionOffsets`——ISS-69 修 sanitize innerHTML 重写路径，与本 bug 独立。

**验证**：e2e `first-selection-action.spec.ts` 4 case（SVG sanitize / onerror / dblclick / 连续两次删除）；回归 mermaid-ir-renders / rich-media-cross-surface 全过；dangerous-boundaries 1 fail 与 main baseline 一致（既有 flake）。

### [DEC-135] - 2026-08-12 - 文档/窗口切换后阅读位置重置到顶部（PR #112）

**背景**：Tab/Window 切换触发 CodeMirror `setValue` 或 Vditor 销毁重建，scrollTop 被重置但未按 tabId 独立保存。

**决策/结论**：`AppLayout.tsx` 加 `Map<tabId, number>` 内存缓存 + capture 阶段 scroll 持续跟踪 + `useLayoutEffect` 同步抑制 setValue 触发的 scroll-to-0 + rAF 重试恢复。

**验证**：新增 `e2e/scroll-restore.spec.ts` 覆盖源码/WYSIWYG/per-tab 隔离 3 case，断言真实 scrollTop 数值（非「没崩」假阳性）。回归 mermaid-ir-renders / rich-media-cross-surface / dangerous-boundaries 全过。

### [DEC-134] - 2026-08-12 - http:// 图片被拦截显示「图片协议被阻止」（PR #110）

**背景**：`tauri.conf.json` 的 CSP `img-src`/`media-src` 白名单只有 `https:` 没有 `http:`，WKWebView 拒绝加载 http 图片；`classifyError` 把 http 失败误归 `blocked-scheme`。

**决策/结论**：CSP 的 `img-src`/`media-src` 补独立 `http:` token（图片/音视频低风险）；`connect-src`/`script-src`/`frame-src`/`font-src` 保持严格不放 http（与 ISS-178 一致）；修正 `classifyError`；`src-tauri/Info.plist` 配 `NSAppTransportSecurity`（`NSAllowsArbitraryLoads`）放行 WKWebView http（CSP 编译期烘焙、settings 仅 localStorage，运行时开关不可行，故改默认值）。

**验证**：`tauriCapabilities.test.ts` 的 `http:` 守卫 + Info.plist ATS 守卫；npm test 597/597。**真机验证（NOT_VERIFIED）**：CSP + ATS 需 release 构建生效（`tauri build` 合并 Info.plist，`tauri dev` debug 裸二进制可能读不到 ATS）。

### [DEC-133] - 2026-08-12 - Word 纸张预览与微信预览表格列宽问题（PR #105）

**背景**：PR #104 修好主编辑器与阅读预览后，Word 纸张预览与微信预览这两个保真面仍用 `table-layout: fixed` + 单元格 `overflow-wrap: anywhere`，长内容列挤塌短内容列。

**决策/结论**：两个面对齐 PR #104 修复模式——`table-layout: auto` + 单元格 `word-break: normal` + `overflow-wrap: break-word`。保真边界：Word 纸张预览保留 ISS-182 的 `display: table !important`；微信面 `DEFAULT_WECHAT_CSS`（预览面板）与 `DEFAULT_INLINE_STYLE_RULES`（复制进公众号内联）两处同步。

**验证**：真机验证（`tauri dev` + computer-use 驱动 WKWebView）长内容列最宽、版本/日期单行横排不塌缩；typecheck 干净、全量 596/596 单测绿。

### [DEC-132] - 2026-08-12 - 标题行光标漂移 + 复制加粗文本残留 `**`（PR #106 / PR #107）

**背景**：(PR #106) 标题节点的 `# ` marker 被 Vditor 加 `vditor-ir__node--expand`，CSS 把 marker 从 0 宽经 150ms transition 渐变到可见宽，整段标题文本偏移、光标视觉漂移；原 RAF 方案对方向键失效（Vditor 在异步 selectionchange 才加 --expand，RAF 跑在前面）。（PR #107）Vditor IR 把 `**` 等 marker 渲染成 `.vditor-ir__marker` span 用 `width:0;display:inline-block;overflow:hidden`（非 display:none）视觉隐藏，DOM 文本 `**` 始终存在，浏览器 `Selection/Range.toString()` 把这种「视觉隐藏但非 display:none」文本带进纯文本。

**决策/结论**：(PR #106) 改用 `MutationObserver` 监听 IR DOM class 变化，标题节点一旦被加 `--expand` 立即在其 microtask（同渲染帧、绘制前）移除——时序无关，加/移之间无 paint，transition 不触发；保留原 RAF/220ms timer 处理粗体/斜体 marker。(PR #107) 新增 host 级 `copy` 事件拦截：对选区 `range.cloneContents()` 得 DocumentFragment，移除其中所有 `.vditor-ir__marker`（保留 strong/em/a/code 语义标签），清理后 fragment 的 textContent 作 text/plain。

**验证**：1 项单测覆盖「标题 --expand 被立即移除」「粗体/斜体 --expand 保留」；1 项单测覆盖「text/plain 无 `**`、text/html 保留 `<strong>`、marker 已移除」。**真机验证（端到端 NOT_VERIFIED）**：orca/osascript 合成 Cmd+C 在 WKWebView 走原生 copy 绕过 JS handler，复制 handler 端到端须物理键盘确认；handler 逻辑以单测为准。真机（PR #106）tauri dev + orca computer-use 截图确认 marker 折叠不可见。

### [DEC-131] - 2026-08-12 - Markdown 表格列宽被强制等分、长内容列挤塌短内容列

**背景**：编辑器（`app.css`）与阅读预览（`preview.css`）表格规则同时设 `table-layout: fixed`，单元格用 `overflow-wrap: anywhere`——fixed 把所有列强制等宽，anywhere 把单元格最小宽度降到 1 字符。长内容列吞掉几乎所有宽度，短内容列挤成逐字竖排、行高极高。

**决策/结论**：改为 `table-layout: auto`（列宽按内容自适应）+ 单元格 `overflow-wrap: break-word`、`word-break: normal`（最小宽度回到「最长词」，CJK 由 normal 在任意两字间断行）。Word 纸张预览与微信预览存在同根因但属不同保真面，本期未改，留作后续（后由 DEC-133 修）。同一次排查的 `<details>` 折叠失效结论为 Vditor IR 已知限制（记为限制不强行修），二维码不渲染在当前 main 已正常。

**验证**：真机验证（`tauri dev` + orca computer-use 驱动 WKWebView）：同一张「最近更新的 Skill」表 Skill 列链接由逐字竖排恢复为横向一行，「更新要点」列合理最宽（约 40-50%），行高正常。

### [DEC-130] - 2026-07-31 - 关闭未保存文档或退出应用时增加保存确认（#68）

**背景**：文档存在未保存修改时，关闭标签、关闭窗口或退出应用（Cmd+Q / 红绿灯 / 窗口 X）前无保存确认，直接丢失。原有原生 `window.confirm` 的单标签关闭确认也需升级。此前 DEC-108「关窗前 dirty confirm」因 Tauri 2.11 close 行为延后，本期以 Rust `prevent_close` + 事件方案闭合。

**决策/结论**：三选项确认框——「保存」（保存后继续关闭）、「不保存」（放弃修改关闭）、「取消」（返回编辑器）；文档已保存时直接关闭不打扰。退出应用时多个未保存标签逐个确认，中途取消任一个即终止退出。原生 `window.confirm` 升级为该三选项对话框；窗口关闭拦截走 Rust `prevent_close` + 事件方案（避免此前前端 `onCloseRequested` 在 macOS 误拦截）；程序化关闭路径（merge-back、确认后关闭）改走 `destroy()` 绕过拦截。

**验证**：Issue 列出的「批量关闭」「切换文档替换编辑内容」两个场景留作后续。（CHANGELOG 该条目未列具体测试数字，完整验证见 PR #68。）

### [DEC-129] - 2026-07-30 - 跨父目录的本地相对路径图片 WYSIWYG 不显示（ISS-187）

**背景**：Vditor 的后处理清洗会移除已转换好的 Tauri `asset:` 图片地址，但保留原始 Markdown 链接标记，导致 `../../figures/...` 这类合法路径在 Obsidian 可见、在 Folia 中丢失。

**决策/结论**：从 Vditor IR 标记恢复原始相对地址，重新经过现有本地资源解析与敏感路径检查后生成可加载 URL；同时监听 Vditor 初始化、外部 `setValue()` 和清洗引起的媒体节点替换。未通过全局放行 `asset:` 或扩大资源 scope 绕过安全边界。

**验证**：（CHANGELOG v0.6.3 该条目未列具体测试数字，完整验证见对应 PR。）

### [DEC-128] - 2026-07-28 - 长文章点击侧边 TOC 跳到错误标题或停在中间（ISS-186）

**背景**：TOC、源码模式和 WYSIWYG 跳转各自维护标题定位逻辑，不共用模型，长文跳转定位不准。

**决策/结论**：TOC、源码模式和 WYSIWYG 跳转改为共用 `tocService` 标题模型；解析器按行识别 Markdown ATX 标题并排除 fenced code 中的 `#` 示例，DOM 绑定只选择 Vditor 真实标题、排除 HTML / 图表预览标题，并为重复标题按顺序绑定独立 `toc-N` 锚点。超过约 1.5 个编辑器视口的跳转改为即时定位，近距离仍保留短平滑、同时尊重「减少动态效果」。

**验证**：新增单元测试与 30 节长文逐像素 E2E，覆盖反引号 / 波浪线围栏、HTML heading、重复标题、源码位置和远距离跳转。

### [DEC-127] - 2026-07-28 - 设置页栏目切换时内容区闪一下（ISS-185）

**背景**：非默认栏目按需加载，但点击后立即卸载当前内容、显示 `SectionFallback`，造成「闪一下」。

**决策/结论**：左侧选中态立即响应，右侧通过 deferred section 保留上一栏目，目标 chunk 就绪后一次性提交新内容（不再立即卸载当前内容）。鼠标移入、键盘聚焦和点击栏目时主动预热对应 chunk，不增加应用首屏静态包。

**验证**：新增延迟 EditorSection chunk 的逐帧 E2E，断言加载期间旧「通用」内容始终可见、`.settings-section-loading` 全程 0 帧，随后只切换为真实「编辑器」内容。

### [DEC-126] - 2026-07-28 - 移除工具栏「插入法律文档模板」按钮

**背景**：v0.6.0 在「显示源文件」左侧新增的「插入模板」按钮（ClipboardList 图标）经真机使用反馈冗余——法律文档模板入口对日常 Markdown 阅读场景非必需，且首页未打开文件时以禁用态占位，被感知为「多出来的按钮」。

**决策/结论**：移除该按钮及其下拉菜单、CustomEvent 接线（Toolbar 派发 + WysiwygEditorPane 监听）、`.toolbar-template-*` CSS 与三语 `toolbarInsertTemplate*` i18n key；模板源数据 `src/services/legalTemplates.ts` 保留备用，未来需要时可重新接回。工具栏回到「打开 / 保存 / 另存 / 源码 / Word 预览 / HTML 预览 / 设置」简洁布局。

**验证**：（CHANGELOG v0.6.2 Changed 该条目未列具体测试数字。）

### [DEC-124] - 2026-07-26 - 设置页首次打开白屏/骨架闪烁闭合（ISS-180）

**背景**：用户首次点击设置后约 300ms 内只能看到对比度极低的骨架，近似白屏。根因：① 默认「通用」section 与其它 section 一样走 `React.lazy`，形成外层 SettingsPage + 内层 GeneralSection 双层 Suspense；② 骨架背景对比度过低。v0.6.0 修了内层，但外层 SettingsPage chunk 首次加载等待仍在——`preloadSettingsPage()` 是 fire-and-forget，`setSettingsVisible(true)` 立即触发 fallback。

**决策/结论**：(v0.6.0) 默认 section 改为静态导入、移出内层 Suspense；骨架对比度提升。(v0.6.1 续修) `SettingsPage` 外壳从外层 `React.lazy` 改为静态导入，剥掉外层 `<Suspense fallback>`；7 个非默认 section 仍按需 lazy。`Cmd+,` 与工具栏「设置」改为 `preloadSettingsPage().then(() => setSettingsVisible(true))`。

**验证**：E2E 重写为首帧非 fallback 契约——MutationObserver 截 `.settings-overlay` 首次出现那一帧，断言 `.settings-modal-skeleton` 0 帧、真实「设置 / 通用 / 4 行控件 / 8 nav」同帧齐备。ISS-152 用例新增 `settings-modal-skeleton=0` 断言；ISS-153 删掉旧 skeleton 等待。

### [DEC-123] - 2026-07-26 - Word 纸张预览配置映射消除预览/导出差异（ISS-182，双管线）

**背景**：Word 纸张预览与实际 DOCX 导出在表格边框、标题粗体、表格布局、页码上有四处「可模拟但当前映射错误」的差异，误导用户。

**决策/结论**：按「快速 HTML 模拟 + 权威 DOCX 导出」定位，修正 4 处差异，不动 DOCX 导出管线。① 表格边框宽度按预设 `table.border_width` 映射（此前写死 1px）；② 标题粗体由预设 `bold` 字段驱动（此前 H1-H4 无条件硬编码粗体，覆盖了 `bold:false`）；③ `display:table !important` 压过 Vditor 的 `.vditor-reset table{display:block}`；④ 按 `page_number` 配置在纸张页边距区域渲染页脚/页眉页码节点（此前完全不渲染）。另：超高内容块（单块超一页）产生 `content-overflow-truncated` 诊断并告警「导出会完整保留」。

**验证**：新增单测覆盖表格边框、标题粗体、页码格式化与节点渲染。

### [DEC-122] - 2026-07-26 - 移除 Toolbar「插入图片」按钮

**背景**：v0.5.0 Wave-1 在一级工具栏加的图片插入按钮经真机使用反馈冗余——粘贴 / 拖入图片已由 `MediaInsertionService` + `ImageAssetStore` 完整覆盖，单独按钮违反「工具退到背景层」设计原则且挤占工具栏。

**决策/结论**：移除按钮及全部接线（`handleInsertImage` / Tauri 文件选择器逻辑 / `TOOLBAR_INSERT_IMAGE_EVENT` CustomEvent / `WysiwygEditorPane` 监听 / `guessMimeFromName` helper / 三语 i18n / Toolbar 事件测试），保留 paste/drop 受管图片路径。Toolbar 回到简洁布局。

**验证**：（CHANGELOG v0.6.0 Changed 该条目未列具体测试数字。）

### [DEC-121] - 2026-07-20 - 受管图片资源骨架 imageAssetService（ISS-179 Phase 3）

**背景**：粘贴/拖入图片插入临时 `blob:` object URL，重启后失效；需受管资产骨架支撑落盘。

**决策/结论**：新增 `src/services/imageAssetService.ts`：sha-256 hash 去重、`sanitizeFileName` / `resolveAssetFileName` 纯函数、pending↔persisted state machine、object URL 与相对路径切换。Phase 3 后续 Rust asset scope / Vditor toolbar 接入留给独立 PR；最终落盘（保存前把图片字节写入 `文档名.assets/` 并把 `blob:` 替换为相对路径）由 v0.6.0 DEC-119 决策 6/7 完成。

**验证**：14 个 vitest 用例覆盖。

### [DEC-120] - 2026-07-20 - 富媒体统一渲染协调器 RenderCoordinator（ISS-179 Phase 1）

**背景**：富媒体（mermaid/echarts/math 等）异步渲染与 artifact 生成存在竞争，导致跨 surface 渲染不一致（主 IR 含 SVG / HTML 复制无 SVG / Word 预览 svg=0）。

**决策/结论**：新增 `src/services/renderCoordinator.ts`，`createRenderCoordinator()` 工厂 + `renderMarkdownArtifact(source, options)` 契约（surface / filePath / generation / signal）。generation 单调递增，旧 generation 完成被丢弃；AbortSignal.abort() 让当前 generation resolve 为 aborted artifact；5s 软超时返回 timeout diagnostics；MutationObserver 等待 `.language-mermaid` SVG / `.language-math` KaTeX 终态，不再依赖 `after()` / `data-render="1"`。`wordPreviewArtifactService` / `WechatPreviewPane` / `WordPaperPreviewPane` 改造走 coordinator。

**验证**：Phase 0 红测试 4 vitest + 3 Playwright 全部转绿，原 388 vitest 保持绿，typecheck / lint / build 全绿。Playwright 关键转变：HTML 复制从 graph TD 源码 → 完整 mermaid SVG；Word 预览 `panelHasGraphTd` true → false；跨 surface 一致性 `wechatHasGraphTd` true → false。

### [DEC-119] - 2026-07-20 - 富媒体统一渲染治理（ISS-179 Phase 0~4，含 §9.2 Mermaid 补记）

**背景**：2026-07-12 真实 Tauri v0.4.7 生产探针显示富媒体跨 surface 渲染不一致。需统一渲染契约与 fixture。

**决策/结论**：(Phase 0) 新增 `fixtures/rich-media/`（13 Markdown 场景 + 7 资产 + manifest）+ 确定性红测试。(Phase 1) RenderCoordinator（DEC-120）。(Phase 2) `WysiwygEditorPane` `input()` 回调在 sanitizeIrDom 后立即调 `resolveLocalImages`，让粘贴/拖入相对路径图片无需重开即显示；新增 `e2e/rich-media-fixture-matrix.spec.ts` 6 fixture。(Phase 3) imageAssetService（DEC-121）。(Phase 4) `.github/workflows/ci.yml` 新增 `playwright` job。(决策 6/7，v0.6.0) 粘贴/拖入图片保存前把字节写入 `文档名.assets/`，`blob:` 替换为相对路径，重启正常显示；路径解析在 Rust 侧完成，复用 `is_denied_root` + 新增 `..` 遍历防护。(§9.2 补记) Mermaid flowchart `htmlLabels:true` 把节点文字放 `<foreignObject>`，DOMPurify svg profile 默认剥除导致文字丢失；显式 `ADD_TAGS:['foreignObject']` 保留，内部 HTML 仍按 html profile 清洗。

**验证**：Phase 0 4 vitest + 3 Playwright 红→绿；Phase 2 e2e 6 fixture 端到端就绪断言；v0.6.0 `e2e/mermaid-fidelity.spec.ts` 3 用例 + sanitizeService 2 用例；CI 矩阵 10/10 全绿；vitest 408/408（v0.5.0）。

### [DEC-118] - 2026-07-07 - 主编辑器 IR 模式 Mermaid/ECharts/KaTeX 等围栏不显示（ISS-63 / PR #64）

**背景**：v0.4.5 / v0.4.6 桌面包里含 mermaid 等围栏的文档在主编辑器只显示源码不渲染。根因：`sanitizeVditorIrHtml` 在 after()/input()/setValue() RAF 回调中同步用 DOMPurify 整体重写 IR DOM，与 Vditor 内部异步代码块渲染器产生 detached-node 写入竞争——folia sanitize 跑完后旧节点全 detached，Vditor 异步加载完成调 `item.innerHTML = svg` 写到了 detached 节点。是 v0.4.4/v0.4.5（DEC-112/114 修 SVG 渲染）引入的回归。

**决策/结论**：方案 A + B 组合。方案 A：DOMPurify 处理前后保留 `.vditor-ir__preview[data-render="1"]` 的 innerHTML（还原前再过一遍 sanitizeForVditor 防恶意 svg 绕过）。方案 B：`sanitizeIrDom` 完成后重跑 Vditor 静态渲染方法（mermaidRender / mathRender / flowchartRender / plantumlRender / graphvizRender / markmapRender / mindmapRender / chartRender / abcRender / SMILESRender），cdn/theme/math options 从 editor 实例动态拿（避免 hardcoded 主题不一致），`editor.constructor` 拿 Vditor 类引用避免二次 await import flake；try/catch 防 unhandled rejection + 卸载竞态检查。

**验证**：`e2e/mermaid-ir-renders.spec.ts` 修复前 `hasSvg: false`，修复后 `hasSvg: true, svgCount: 1` 且 preview innerHTML 含完整 flowchart SVG；`npm test`（47 文件 / 388 测试，3 连稳）/ typecheck / lint / build / cargo check 全绿。

### [DEC-116] - 2026-06-26 - HTTPS 图片（含 WebP）在主编辑器/预览窗不显示（ISS-178 / PR #62）

**背景**：`tauri.conf.json` 的 CSP `img-src` / `media-src` 只放行 `'self' asset: http://asset.localhost data: blob: file:`，任何 `https://` 来源的 `<img>` / `<source>` / `<video>` 被 WebView 拒绝加载——`<img>` 节点能进 DOM，但浏览器拦截图片数据。用户报告场景是腾讯云 COS 上的 WebP，根因与 WebP 无关，是 CSP 不允许 `https:`。

**决策/结论**：`img-src` / `media-src` 加上 `https:`；`connect-src` / `frame-src` / `font-src` 维持原状，避免放开脚本 fetch 与 iframe 来源。`tauriCapabilities.test.ts` 增加 `img-src`/`media-src` 含 `https:` 两条断言守住。

**验证**：Vite dev + Playwright 探针注入与 Tauri 一致 CSP，修复前 3 张 HTTPS 图 naturalW=0 + console 4 条 CSP error；修复后 COS WebP naturalW=1280×720、gstatic WebP naturalW=550×368、console 0 error；Tauri dev 实测 4 种引用方式全正常。npm test 47/388、typecheck/lint/build、cargo check 全绿。

### [DEC-114] - 2026-06-23 - 多行 SVG 后方白色条 + SVG 文本继承等宽字体（PR #61）

**背景**：Vditor IR 在清洗后会把部分 SVG 子片段（尤其 `<path>`）降级成普通段落，旧 SVG 修复器只沿相邻 IR HTML 节点收集，遇到普通段落就停止，导致图后方残留空白源码块；部分 AI 生成 SVG 未指定 `font-family`，在 Vditor IR 预览中继承代码块 `<pre>` 等宽字体，长英文标签比原设计宽，看起来像被框截掉。

**决策/结论**：source-aware SVG 修复按原始 Markdown SVG 源码顺序继续识别并隐藏后续残留片段，同时保留图注和正文；修复后的 SVG 预览使用阅读正文字体而非等宽字体。

**验证**：Playwright 注入用户 `ch07.md` 实测 7 张 SVG 全部恢复且无可见残留，session 仍保持未修改；「初版 Skill 的文件结构」根节点文字宽度由约 285px 降为约 226.6px，可正常落在 240px 蓝框内。

### [DEC-112] - 2026-06-22 - Markdown 内联 SVG 在主编辑器/HTML 预览/Word 预览被截断或变白（ISS-176）

**背景**：Vditor IR / Lute 会把漂亮排版的多行 SVG 拆成多个 `html-block`，部分 SVG 的 marker 还会被截成只有背景 `<rect>` 的片段；旧清洗逻辑会把这些片段补闭合并回写到会话，导致右侧预览也拿到污染内容。

**决策/结论**：按原始 Markdown SVG 块修复 IR 可见预览，安全跳过无害 SVG 片段 marker 的单独清洗，并在 Vditor preview 前用占位符保护完整 SVG；HTML/Word 预览会移除 Vditor 复制按钮等 preview chrome。

**验证**：Playwright Chromium 注入用户 `ch07.md` 实测：主编辑器、HTML 预览、Word 预览测量层和分页层均为 7 张完整 SVG，初始化后 session 仍保持干净。

### [DEC-111] - 2026-06-21 - drag tab 到空白处创建新独立窗口

**背景**：DEC-110 移除 tear-off 按钮后，需补齐「drag 一个 tab 到空白处 → 自动创建新独立窗口并把该 tab 从源窗口移除」的浏览器范式入口。

**决策/结论**：dropEffect === 'none' 触发 `tearOffViaDrag`；同窗口 drag 不误触发 tear-off（drop accepted）。新增 `TabBar.handleDragEnd` + `useSession.tearOffViaDrag` callback + `AppLayout` 接线。**遗留（DEC-108）**：dirty 拦截整体方案需走 Rust `OnCloseRequested` + `prevent_close()` 重做——当前 dirty tab 关窗时经 `window:closed` 事件回到主窗口，dirty 标记保留在缓存里无数据丢失。

**验证**：（CHANGELOG v0.4.2 该条目未列具体测试数字。）

### [DEC-110] - 2026-06-21 - tear-off 改为纯 drag，移除「弹出此标签」按钮 + toolbar X 关闭按钮（ISS-174 follow-up）

**背景**：用户反馈 tear-off 体验与浏览器不一致——tab 上 ⤴ 按钮 + 独立窗口 toolbar 右侧 X 关闭按钮让独立窗口看起来像「附属页面」。

**决策/结论**：tear-off 仅靠 HTML5 drag；关闭独立窗口走 OS 原生红绿灯 / Windows 标题栏 X（与浏览器一致）；drag-out 仅在源窗口 ≥2 tab 时启用（单 tab 窗口禁 drag，避免 drag-out 后源窗口变空）。清理 dead code：`tabWindowService.confirmCloseWindowWithDirty` + `useSession.tearOffTab` 包装层 + `sessionReducer` 'tearOffTab' action。另（ISS-174）：`create_tab_window` builder 补齐与主窗口一致的窗口装饰（macOS `TitleBarStyle::Overlay` + `hidden_title(true)` + `traffic_light_position(16, 16)`，Windows/Linux 显式 `decorations(true)`），消除 NSWindow 标题栏分隔白线。

**验证**：（CHANGELOG v0.4.2 该条目未列具体测试数字。）

### [DEC-108] - 2026-06-21 - 关窗前 dirty confirm（延后，后由 DEC-130 实现）

**背景**：v0.4.2 / v0.4.3 发现 `useSession` 注册的 `getCurrentWindow().onCloseRequested(() => { flush(); })` handler 在 macOS Tauri 2.11.0 上误拦截 close——即便未调用 `preventDefault()`，窗口也不再自动 destroy。若需 onCloseRequested 拦截做 dirty confirm，需先研究 Tauri 2.11 的 close 行为或显式调用 `window.destroy()`。

**决策/结论**：本期延后——移除 JS onCloseRequested 注册；Rust `on_window_event(CloseRequested)` handler 已承担独立窗口 tab 回收 + `window:closed` emit，不阻塞关窗。当前 dirty tab 关窗时经 `window:closed` 事件回到主窗口，dirty 标记保留在缓存里，无数据丢失。DEC-108 仅是关窗前 confirm 的 UX 打磨，后由 DEC-130（v0.6.4 #68）以 Rust `prevent_close` + 事件方案正式实现保存确认三选项对话框。

**验证**：本期未实现该 confirm，NOT_VERIFIED（关窗不丢失数据的兜底由 session 缓存保证）。

### [DEC-106] - 2026-06-20 - 恢复 macOS 自动更新

**背景**：v0.4.0 `bundle.targets: ["dmg", "nsis"]` 删掉了 macOS 的 `"app"` target（updater binary），且 `release.yml` 的 `includeUpdaterJson: false` 让 tauri-action 不生成 .sig——双重原因导致 macOS runner 没生成 `.app.tar.gz` / `.app.tar.gz.sig`，publish job `gh release download --pattern "*.sig"` 拿不到，latest.json 缺 darwin-aarch64 / darwin-x86_64 entry，应用内「检查更新」无法拉到 v0.4.0。

**决策/结论**：`bundle.targets` 加回 `"app"` + `includeUpdaterJson` 改回 `true`。

**验证**：3 次 CI 重打（每次都有真实进展）后发布成功，9 个产物齐全，v0.4.1 latest.json 三平台签名齐全。

### [DEC-104] - 2026-06-20 - IR sanitize 回归与 tear-off 窗口 tab 过滤（ISS-170）

**背景**：`WysiwygEditorPane` 的 IR sanitize 此前只清理可见 preview DOM，不清理隐藏的 `code[data-type="html-block"]` marker 文本，`VditorIRDOM2Md()` 保存时从 marker 还原 `<script>` / `onerror` / `onload` 等危险源码；`input()` 保存链路用 Vditor 回调传入的旧值写回 session。另：tear-off tab 独立窗口启动时未按 tab 过滤共享 session，会恢复整套标签。

**决策/结论**：IR sanitize 同时清理可见 preview DOM 和隐藏 marker 文本；`input()` 保存链路改用 sanitize 后的当前 `editor.getValue()`。tear-off 独立窗口 URL 新增 `tabIds` 查询参数，启动时按指定 tab 过滤。review follow-up 三点：① `bootstrapSessionForWindow` 不因 `initialTabIds` 空数组回退主 session，tab-window 缺 `tabIds` 或全部失配一律返回占位 tab（安全边界）；② `input()` 复杂表分支 sanitize 命中时跳过 `serviceReplaceHtmlTableBlock` 注入 `original.html`，防 DOMPurify 刚剥离的 onclick/onerror 被反向灌回（XSS bypass）；③ 5 处 `sanitizeIrDom` 调用包 try/finally、3 处 RAF 回调检查 cancelled。

**验证**：新增真实 Lute round-trip、组件 input 保存、session bootstrap、tabIds URL 解析与 Rust URL 单测；Vite dev + Playwright 实测 tab-window 只显示目标标签，危险 HTML 持久化后保留 svg/rect 并剥离危险内容；新增 2 个 bootstrap（空 tabIds/全失配走占位）+ 2 个 WysiwygEditorPane（sanitize 命中跳过 restore + 卸载后 RAF 不抛错）测试。

### [DEC-102] - 2026-06-20 - 多窗口 tear-off tab / merge-back tab（ISS-164）

**背景**：需支持主窗口 tab 拖出成独立窗口，独立窗口 tab 拖回主窗口合并。

**决策/结论**：Rust `create_tab_window` command 用 `WebviewWindowBuilder` 创建独立窗口（URL `?mode=tab-window&label=...`）；独立窗口 tab bar 拖 tab 回主窗口 → 合并回主窗口 + 源窗口空则自动关闭。session 方案 1（YAGNI）：保持前端 `useSession`（useReducer + localStorage），不把 session 移到 Rust，窗口间通过 Tauri event bus 同步 `tab:tear-off` / `tab:merge-back` / `session:full-sync` / `window:closed` / `tab:drop-requested`，last-write-wins 持久化；Rust 只追踪 `label → tabIds` 映射用于关闭时回收。新增 `tabWindowService.ts`（IPC 封装）、`useTabWindowSync.ts`、`tabDragPayload.ts`；`sessionReducer` 新增 tearOffTab/removeTabById/receiveTab/windowClosed actions；capabilities 加 `core:webview:allow-create-webview-window` + `tab-window-*` glob。**不在本期**：跨独立窗口拖 tab、精确 drop index、session 移 Rust、位置记忆、macOS WKWebView 实测。

**验证**：`tabWindowService.test.ts` 27 个单测（监听/emit/反注册/常量）；Rust 8 个新单测（label 校验、Mutex 读写、urlencode、多窗口共存）。

### [DEC-099] - 2026-06-20 - 预览 sanitize 加固，消除 onerror/onload 理论窗口（ISS-169）

**背景**：ISS-168 用 Vditor `after()` 回调对已写入 DOM 的 HTML 做 DOMPurify 后处理，依赖 `after()` 在浏览器异步加载 onerror/onload 之前同步触发——通常成立但理论上非绝对安全。

**决策/结论**：sanitize 改在 Vditor.preview 的 `transform(html: string): string` 钩子里完成（Vditor 在 `previewElement.innerHTML = html` 之前同步调用 transform），对 Lute 已转义的 HTML 用 `sanitizeForVditor`（DOMPurify `USE_PROFILES: { html, svg, svgFilters }`）做 sanitize，再让 Vditor 写入 DOM——危险元素从未以「危险态」插入 DOM，从源头消除 onerror 窗口。`after()` 钩子仍保留给「本地图片解析」与「toc id 注入」，不再调用 sanitize 后处理。`sanitizeForVditor` 本身未改（ISS-168 的 6 个测试继续覆盖）。

**验证**：新增 `PreviewPane.test.tsx` 4 个测试（transform 钩子已注册、剥离 `<script>`/`onerror`、与 `sanitizeForVditor` 行为等价、`after()` 不再触发 sanitize）。**真实 Tauri WebView 复测建议**：按 PR 描述用 `<img src="x" onerror="...">` 测试块在桌面端复测确认 DOM 永远观察不到危险态。

### [DEC-098] - 2026-06-20 - 本地相对路径资源解析路径遍历防护与扩展覆盖范围（ISS-160）

**背景**：`resolveLocalResourcePath` 无敏感路径防护，前端或被 XSS 注入的代码可能用合法后缀 `.md` / `.html` 旁路读取 `/etc/passwd` / `.ssh` 等敏感文件；`resolveLocalImages` 此前仅覆盖 `<img src>`。

**决策/结论**：**越界防护**——`resolveLocalResourcePath` 新增 `isSensitivePath` 黑名单，拒绝解析后落在敏感系统/凭据目录（`/etc` `/System` `/var` `/usr` `.ssh` `.gnupg` `.aws` `C:\Windows` 等）的相对路径；保留合法 `../` 上级引用（律师文档常把图片放共享上级 `证据/` 目录）。**扩展范围**——`resolveLocalImages` 从仅 `<img src>` 扩展到 `<source src>`、`<video poster>`、`<img>`/`<source>` 的 `srcset`、CSS `background-image: url(...)`（inline `style` + `<style>` 块）。外部/越界/无法解析的资源保留原属性、不抛错。

**验证**：（CHANGELOG v0.4.0 Changed 该条目未列具体测试数字；敏感路径黑名单后由 ISS-172 的 `read_opened_document`/`write_opened_document` 共享 `DENY_PATH_PREFIXES` 加固。）

### [DEC-097] - 2026-06-20 - Word 预览面板纸张两侧明显留白、内容区被挤压（ISS-166）

**背景**：`WordPaperPreviewPane` 的 `PREVIEW_HORIZONTAL_PADDING = 56` 预留过多，纸张缩放后宽度 = 面板宽 - 56，仅占面板约 88%；叠加 `.word-preview-scroll` 内边距偏大。

**决策/结论**：`PREVIEW_HORIZONTAL_PADDING` 56→16（匹配 scroll 左右 padding 8px×2）+ `.word-preview-scroll` padding `18px 18px 40px`→`10px 8px 22px` + `.word-preview-pages` gap `22px`→`12px`。

**验证**：node + playwright 截图实测纸张宽度 404px→444px，基本占满面板（460-16=444）。

### [DEC-096] - 2026-06-20 - 本地相对路径图片完全不显示（Tauri asset 协议三处配置缺失）

**背景**：`convertFileSrc()` 生成的 asset URL 被三重拦截：① `src-tauri/Cargo.toml` 的 `tauri` crate 未启用 `protocol-asset` feature（Rust 端不编译 asset protocol handler）；② `tauri.conf.json` 无 `assetProtocol.enable/scope`（默认 scope 空，拒绝所有路径）；③ CSP `img-src 'self' data: file:` 不含 `asset:` / `http://asset.localhost`。`localImageResolver` / `resolveLocalResourcePath` 代码本身正确，此前纯粹是 Tauri 配置缺失。

**决策/结论**：补齐三处配置——`protocol-asset` feature + `assetProtocol: { enable: true, scope: { allow: ["$HOME/**/*"], requireLiteralLeadingDot: false } }` + CSP `img-src`/`media-src` 加 `asset: http://asset.localhost`。

**验证**：同目录、跨目录、含中文/空格/emoji 目录名、`%20` URL 编码的相对路径图片均正常加载。

### [DEC-091] - 2026-06-20 - 超长 Markdown 文件（数 MB / 数千行）打开白屏与编辑卡顿（ISS-159）

**背景**：**打开阶段**——Rust `read_opened_document` 返回 `Vec<u8>`，被 Tauri 序列化成 JSON 数字数组（10MB 文件膨胀为 30-40MB、内存峰值达原始文件数倍并卡死 WebView）。**编辑阶段**——`handleContentChange` 中的大纲（TOC）全文正则提取每键触发；active-heading 的 `MutationObserver` 依赖 `file.content`，每次按键都 disconnect 后重新 observe 整棵 DOM。

**决策/结论**：打开阶段——`read_opened_document` 改为返回原始字节 `tauri::ipc::Response`，前端 `invoke` 直接拿到 `ArrayBuffer`，序列化膨胀消除、内存峰值降到约原始文件一倍。编辑阶段——TOC 全文正则提取改为 150ms 防抖（文件内容仍每键同步保存）；active-heading 的 `MutationObserver` 不再依赖 `file.content`。另：超大文件（>10MB）在 Rust 后端用 `metadata` 读取前拦截并返回明确错误，前端弹出原生提示。

**验证**：（CHANGELOG v0.4.0 Performance / v0.3.22 Fixed 该条目未列具体测试数字。）

### [DEC-085] - 2026-06-13 - HTML 阅读预览 / Markdown 预览统一为 Vditor WYSIWYG 一体化（ISS-155）

**背景**：此前 HTML 阅读预览与普通 Markdown 预览是两套独立模式，含状态机（`htmlReadingPreference`）、toolbar 切换按钮与结构化表格编辑器（`HtmlTableEditor`），心智负担重。

**决策/结论**：所有 Markdown 与 HTML 文档默认直接进入 Vditor WYSIWYG（`mode: 'ir'`），普通段落与不含 `rowspan` / `colspan` 的简单表格内文字可直接编辑；含 `rowspan` / `colspan` 的复杂表格区域在 Vditor 中标记为 `contenteditable="false"` + `data-folia-locked="table"`，结构与文字均不可改，输入回调对比原 `findHtmlTableBlocks` 自动恢复被改动的复杂表格源码。删除 `htmlReadingPreference` 状态机、`canToggleHtmlReadingPreview` 派生、HTML 阅读预览 toolbar 切换按钮、结构化表格编辑入口 `HtmlTableEditor` 组件（用户确认不使用结构化编辑），Toolbar 源码按钮作为兜底编辑入口。复杂表格上方 hover 出现「查看原貌」小图标弹出忠实 HTML 版本。

**验证**：（CHANGELOG v0.3.22 Changed/Removed 该条目未列具体测试数字；新增 `classifyHtmlTableBlocks` 5 个单元测试。）

### [DEC-065 ~ DEC-084 区间] - 2026-05-19 ~ 2026-06-13 - v0.3.10 ~ v0.3.21 早期决策（CHANGELOG 未逐条标 DEC 号，按版本聚合）

> 以下版本的变更在 CHANGELOG 未逐条标注 DEC 编号，其决策正文无法精确复原。按版本聚合列出关键 ISS / 变更摘要，编号归属为推断区间（DEC-065~084），完整根因见 CHANGELOG 对应版本段与 DEC-035~064 中已存在的相关条目。

- **v0.3.10**：官网 Astro 站建设（DEC-046 已记录）、官网 favicon 用应用 logo（DEC-049）、首屏居中（DEC-048）、文案知识工作者定位（DEC-047）；工具栏内联更新按钮/后台下载（DEC-042/045）；日语 ja-JP 完整语言支持；Word 导出表格行高 HeightRule + 单元格边距；配置文件集中到 `config/`（DEC-044）；移除 `UpdateDialog`。
- **v0.3.11**：Word 纸张预览保持快速 CSS 路线（DEC-055）；官网脚本依赖恢复（DEC-053）；长表格正文单元格误按表头样式渲染修复。
- **v0.3.12**：中文优化字体栈（DEC-056）；构建拆包收敛（DEC-057）；DOCX XML 回归（DEC-057）。
- **v0.3.13**：Word JSON schema 扩展（`styles` / `markdown_mapping` / `html_mapping`）；md2word YAML 转 JSON 字段别名兼容（`row_height_cm` / `cell_margin` / `code_block.label` 等）；内置预设「公文报告」贴近 GB/T 9704、「学术论文」贴近 GB/T 7713.2。
- **v0.3.16**：Gitee 附件同步改 best-effort 带超时；系统双击打开（DEC-060）；HTML 直读预览（DEC-060）；Windows MSI 打包修复（文件关联描述改 WiX 兼容文本）。
- **v0.3.17**：系统路径打开 Markdown / HTML 保存修复（DEC-060 续，已保存路径改由 Rust 后端受控写回）。
- **v0.3.18**：字体三组（DEC-063 已记录）；H1-H6 默认跟随正文。
- **v0.3.19**：白屏修复（DEC-064 已记录）。
- **v0.3.20**：官网迁独立仓 `personal-site`（删 `website/` Astro + 转发脚本 + deploy-website.yml）；浮动大纲固定改左侧常驻栏；预览中文字体/英文字体 CSS 变量实时响应（DEC-063 续，修 ISS-136 阅读字体不实时更新）；正文按英文字体栈→中文字体栈→总体阅读字体回退组合。
- **v0.3.21**：浮动大纲横条改轻量查看入口（点击/悬停只展开不固定）+「总是固定大纲」选项；HTML 阅读预览顶部加「退出 HTML 预览」切换；源码模式 TOC 跳转 CodeMirror 修复。

### [DEC-064] - 2026-06-04 - 修复 v0.3.18 桌面空白与源码模式白屏回归

**背景**
用户反馈最新版本直接打开、双击打开文件和右键打开文件后主页面空白。复验 GitHub 最新 Release `v0.3.18`（发布提交 `6d78a35`）时，生产网页预览普通首屏可显示，但点击“源码模式”后页面会清空，浏览器报 `TypeError: Class extends value undefined is not a constructor or null`，错误来源是拆分后的 `editor-vendor` chunk。

**根因**
- `v0.3.18` 的 Vite 配置仍把 CodeMirror / UIW 依赖放在同名 `editor-vendor` 分组并设置 `maxSize`，Rolldown 会继续按体积任意切分 CodeMirror 依赖，生产运行时可能破坏初始化顺序。
- 桌面 `.app` 还需要更稳的前端资源路径。Vite 默认输出 `/assets/...` 绝对路径；在 Tauri 嵌入式 WebView 中，使用 `./assets/...` 相对路径更适合随 `frontendDist` 嵌入加载，避免首窗无法拿到前端 bundle 后表现为空白。

**决策**
- `config/vite.config.ts` 设置 `base: './'`，让生产 HTML 和动态 chunk 资源都走相对路径。
- 保留 CodeMirror 明确拆包方案：`editor-core-vendor`、`editor-language-vendor`、`editor-ui-vendor`，不再对 CodeMirror vendor 使用 `maxSize` 任意切分。
- `src/build/viteConfig.test.ts` 同时约束两件事：生产资源必须使用相对路径；CodeMirror vendor 分组不得回到 `maxSize` 任意拆分。

**验证**
- `npm test -- src/build/viteConfig.test.ts`：新增相对资源路径测试先失败，加入 `base: './'` 后通过。
- `npm run build`：`dist/index.html` 输出 `./assets/...`；CodeMirror 产物为 `editor-core-vendor`、`editor-language-vendor`、`editor-ui-vendor`。
- 生产预览 Playwright 复验：点击“源码模式”后 `.app-layout`、`.app-toolbar`、`.cm-editor` 均存在，`pageerror` 为空。
- `npm run tauri:build:local`：成功生成 `Folia.app` 和 `Folia_0.3.12_aarch64.dmg`。
- `npm test`、`npm run lint`、`npm run test:e2e`、`git diff --check` 均通过。

**影响**
- 新发布包启动时能更稳定加载前端资源，降低主窗口空白风险。
- 源码模式首次打开不再因 CodeMirror chunk 初始化顺序错误清空整个页面。

### [DEC-063] - 2026-06-01 - 重做 Markdown 阅读字体设置与标题层级

**背景**
ISS-131 调研完成后，用户要求继续推进实现。现有设置页仍以“中文优化 / 中文宋体 / Iowan Old Style / Georgia”这类预设暴露阅读字体，且 Markdown H1/H2 使用衬线 display 字体、H3/H4 使用非衬线体系，造成标题层级风格割裂。

**决策**
- Settings / 预览不再把“中文优化 / 中文宋体”作为主路径，改为中文字体、英文字体、标题字体三组选择；每组都有“默认”，中文和英文支持常见系统字体与自定义字体名。
- 阅读字体栈由英文字体栈 → 中文字体栈 → 默认 reading 栈组合生成，避免只优化中文或只优化英文。
...
- Markdown 阅读预览、`.docx` HTML 预览和 Vditor 即时渲染编辑共用新阅读字体变量；Word 纸张预览继续只由导出预设控制。
...
- 本地浏览器打开 `http://127.0.0.1:5173/`，确认 Settings / 预览显示中文字体、英文字体、标题字体三组选择，选择后主界面阅读字体变量即时更新。
- `npm test`
- `npm run lint`
- `npm run build`
- `git diff --check`
...
**影响**
- 新用户默认看到普通“默认”字体入口，不再需要理解“中文优化 / 中文宋体”预设。
- 纯中文长文、中英文混排和标题层级的默认观感更统一。
- 旧用户升级后旧字体选择会迁移到中文/英文/标题字段，不直接丢失偏好。

### [DEC-062] - 2026-06-01 - 修复生产源码模式 chunk 初始化与设置页外壳首开等待

**背景**
用户复验反馈：前面已经针对源码模式和设置页加载发布过多个版本，但当前版本打开源码模式仍有问题，打开设置页仍会经历较长加载。现有单元测试只验证最终能显示，没有覆盖生产构建首次点击时的 chunk 初始化错误和首开等待体验。

**根因**
- 生产构建中 `editor-vendor` 使用 `maxSize` 让 Rolldown 按体积任意拆分 CodeMirror 依赖，首次点击“源码模式”加载拆分后的 editor chunks 时会触发 `Class extends value undefined is not a constructor or null`，导致 CodeMirror 挂载失败。
- 设置页低频分区已懒加载，但 `SettingsPage` 外壳本身仍是 `React.lazy()`；用户在空闲预加载完成前点击设置时，会先看到完整加载骨架，而不是立即进入可操作的“通用”设置。

**决策**
- 取消对 CodeMirror vendor 的 `maxSize` 任意拆分，改为按明确包边界拆成 `editor-core-vendor`、`editor-language-vendor` 和 `editor-ui-vendor`，避免跨 chunk 初始化顺序破坏，同时继续保持单个 chunk 低于 500KB。
- `SettingsPage` 轻量外壳改为同步导入；Word 导出、HTML 导出、授权、关于等低频分区继续在设置页内部懒加载。
- 源码编辑器保留懒加载，但在空闲期、工具栏源码按钮 hover/focus、稳定 HTML 阅读页“编辑源码”入口 hover/focus/click 时提前预加载。
- 新增回归测试：设置页打开时不再显示外壳加载骨架；构建配置不得回到 CodeMirror `maxSize` 任意拆分。

**验证**
- `npm test -- src/app/AppLayout.test.tsx src/build/viteConfig.test.ts src/app/AppLayoutSourceEditor.test.tsx`
- `npm run typecheck`
- `npm run build`
- 生产预览 Playwright 复验：点击“源码模式”出现 `.cm-editor`，无 pageerror；点击“设置”出现“通用”，`.settings-modal-skeleton` 数量为 0。
- in-app Browser / Playwright 复验：`hasSourceEditor=true`、`settingsTitle=通用`、`skeletonCount=0`、`errors=[]`。
- `npm test`
- `npm run lint`
- `git diff --check`
- `npm run test:e2e -- e2e/preview-resources.spec.ts`
- `npm run test:e2e -- --grep "source mode|source edits|settings modal"`

**影响**
- 发布构建中首次打开源码模式不再因 CodeMirror vendor 拆分而白屏或卡死。
- 启动后首次打开设置页直接显示可操作的“通用”设置，不再经历整页外壳加载骨架。
- 设置页低频重模块仍按需加载，不回退到旧的整页重依赖方案。

### [DEC-061] - 2026-06-01 - 记录 Markdown 阅读字体体系重做方向

**背景**
用户反馈 Folia 当前默认阅读字体仍不够好看，并指出设置页里的“中文优化 / 中文宋体”等选项不自然；同时当前 H1/H2 使用衬线 display 字体、H3/H4 回到非衬线/正文体系，导致标题层级在字号、字重和字体风格上出现割裂。用户要求先调研主流 Markdown 阅读器和 Tableau 的字体设置模式，并记录到任务清单。

**调研结论**
- Obsidian、Typora、MarkText、VS Code Markdown Preview 等更常见的模式是按角色拆分字体：界面、正文/阅读、代码/等宽、主题 CSS，而不是按中文/英文拆分。
- Typora 和 MarkText 的默认主题都倾向于统一无衬线正文栈，标题默认继承同一字体族，靠字号、字重、间距和边框建立层级。
- Bear 支持 Text / Headers / Code 分别选择系统字体，说明“标题字体”可以独立暴露，但默认不应造成层级割裂。
- iA Writer 采用少量精心设计的编辑字体选项，并单独处理 CJK Typography 语言；Ulysses、Zettlr 更强调编辑字体、导出字体或主题 CSS 的分离。
- Tableau 官方格式体系按 Workbook、Worksheet、Worksheet Title、Header、Tooltip 等区域或 style element 设置 `font-family`，没有查到官方按中文/英文分别设置字体的模式。

**决策**
- 本轮不直接改实现，先新增 ISS-131 作为 P1 / L2 任务。
- 后续实现方向不是继续堆“中文优化 / 中文宋体”预设，而是提供普通“默认”入口，并评估中文字体与西文字体两个选择框生成同一个阅读字体栈。
- 标题默认应跟随正文或统一使用一个标题字体，禁止再按 H1/H2 与 H3/H4 混用衬线/非衬线；标题层级主要用字号、间距和渐进字重表达。
- Word 纸张预览继续以导出预设为准，不跟随 Markdown 阅读字体设置。

**调研依据**
- Obsidian Settings: https://obsidian.md/help/settings
- Typora Themes: https://support.typora.io/About-Themes/
- Typora GitHub theme CSS: https://raw.githubusercontent.com/typora/theme.typora.io/gh-pages/assets/css/github.css
- MarkText default theme CSS: https://raw.githubusercontent.com/marktext/marktext/develop/packages/muyajs/themes/default.css
- Bear Typography Options: https://bear.app/faq/typography-options/
- iA Writer Settings: https://ia.net/writer/support/basics/settings?tab=settings-mac
- Ulysses editor font: https://help.ulysses.app/en_US/editor/how-do-i-change-fonts
- Zettlr Appearance: https://docs.zettlr.com/en/editor/appearance/
- VS Code Markdown settings: https://github.com/microsoft/vscode/blob/main/extensions/markdown-language-features/package.json
- Tableau workbook formatting: https://help.tableau.com/current/pro/desktop/en-us/formatting_workbook.htm
- Tableau custom themes: https://help.tableau.com/current/pro/desktop/en-us/formatting_custom_themes.htm

**影响**
- `docs/TASKS.md` 新增 ISS-131，后续实现需要同步修改 Settings、settingsService、预览 CSS、Vditor 即时渲染字体变量、设置迁移、测试和 `docs/DESIGN.md`。
- 当前版本用户可见行为不变。

### [DEC-060] - 2026-06-01 - 补齐桌面文件打开事件与 HTML 文件直读预览

**背景**
用户反馈将 Folia 设为 Markdown 默认打开应用后，双击 `.md` 文件只能启动应用，不能直接加载文件；同时含原生 HTML 表格的阅读页切换源码可能出现空白，`.html` 文件预览中也可能残留 HTML 符号、白色源码框，并丢失右对齐、空行等排版语义。

**决策**
- Tauri 打包配置新增 `.md` / `.markdown` / `.html` / `.htm` / `.docx` 文件关联，其中 Markdown 作为默认编辑类文档。
- Rust 桌面壳新增 `pending_opened_paths` 命令：启动参数和 macOS `RunEvent::Opened` 中的文件 URL 会先缓存为待打开路径，再通过 `opened-paths` 事件通知前端。
- 前端启动时先读取系统传入的待打开文件，再决定是否恢复上次打开文件；运行中收到 `opened-paths` 后直接复用 `openPath()` 打开目标文档。
- `.html` 文件阅读预览不再走 `Vditor.preview()` 的 Markdown 渲染链路，改为提取 `<body>` 内容后用 DOMPurify 清洗并直接渲染安全 HTML。
- HTML 阅读清洗保留安全的阅读排版属性：`align` / `valign` 以及 `text-align`、`vertical-align`、`white-space` 内联样式；继续移除脚本、事件属性和不安全样式。
- 为稳定 HTML 阅读页切换源码补回归，确保源码编辑组件收到当前完整源码。

**验证**
- `npm test -- src/services/htmlReadingPreviewService.test.ts src/services/tauriCapabilities.test.ts src/app/AppLayout.test.tsx src/app/AppLayoutSourceEditor.test.tsx`
...
- 本地浏览器打开 `http://127.0.0.1:5173/`，确认主界面正常加载。
- 检查打包后的 `Folia.app/Contents/Info.plist`，确认 `CFBundleDocumentTypes` 包含 Markdown / HTML / Word 文档类型。

**影响**
- 安装新版本后，系统文件关联具备把 Markdown 文件直接交给 Folia 的配置和运行时处理链路。
- 拖拽打开、打开对话框、上次文件恢复逻辑保持兼容；系统双击打开文件时优先打开被双击的目标文件。
- `.html` 文件更接近浏览器阅读结果，同时仍经过安全清洗，不执行脚本。

### [DEC-059] - 2026-05-30 - 设置页低频分区改为二级懒加载

**背景**
用户反馈软件刚启动后第一次打开设置页仍会卡顿。排查确认，已有启动空闲预加载只能提前加载整个 SettingsPage；但 SettingsPage 顶层静态导入 Word 导出、HTML 导出、授权和关于页，使默认“通用”首屏也要解析低频分区及其重依赖。

**决策**
- SettingsPage 保留轻量外壳和常用基础分区同步加载，保证首次打开能直接显示“通用”设置。
- Word 导出、HTML 导出、授权和关于页改为分区级 `React.lazy()` 动态加载，用户进入对应分区时再解析。
- 分区加载等待时复用现有 skeleton 样式，避免出现空白内容区。
- 新增加载边界回归测试，防止低频分区重新回到 SettingsPage 顶层静态 import。
...
**影响**
- 生产构建中 `SettingsPage` chunk 从约 34KB 降到约 10KB。
- 首次打开设置不再预取 `wechatPreviewService`、DOMPurify 和 `wordPreviewStyle` 等导出相关依赖；进入对应分区后功能保持按需加载。
...
- **目标:** 推进 ISS-126，修复软件启动后首次打开设置页的卡顿问题。
- **操作:**
  1. 在独立 worktree `codex/iss-126-settings-lazy` 中推进，先复用已记录的 ISS-126 文档上下文。
  2. 先跑基线：`npm run typecheck`、`npm test` 均通过。
  3. 新增 `SettingsPage.test.tsx`，先用失败测试锁定 SettingsPage 不应顶层静态导入 Word 导出、HTML 导出、授权和关于页。
  4. 将这些低频分区改为分区级 `React.lazy()` 动态加载，并增加分区 skeleton fallback。
  5. 生产构建确认 `SettingsPage` 首次动态导入只关联 SettingsPage / runtime / React，不再关联导出相关重依赖。
- **验证:** `npm run typecheck`、`npm test`、`npm run build` 通过。
- **结果:** ISS-126 已完成并归档到 CHANGELOG / TASKS 进度日志；TASKS 正文继续只保留未完成事项。

### 2026-05-29 (Codex)

- **目标:** 记录设置页首次打开卡顿问题，并按用户口径清理 TASKS 正文中的已完成任务占位。
- **操作:**
  1. 复核设置页加载链路：`AppLayout` 已有启动 idle 预加载和设置按钮 hover / focus / click 预加载，但 `SettingsPage.tsx` 顶层仍静态导入全部设置分区。
  2. 复核生产构建依赖：SettingsPage 动态 import 关联 `SettingsPage`、`ui-vendor`、`wechatPreviewService`、`purify.es`、`wordPreviewStyle` 等 chunks；首屏“通用”仍会承担 Word / HTML 导出等低频分区的解析成本。
  3. 在 `docs/TASKS.md` 新增 ISS-126，记录初步根因、后续拆分方案和验收标准。
  4. 清理 `docs/TASKS.md` 的“待处理”正文，只保留未处理或仍需决策事项；已完成内容交由进度日志和 `CHANGELOG.md` 发布说明归档。
- **验证:** 文档改动通过 `git diff --check`。
- **结果:** TASKS 正文不再堆叠已完成任务；设置页首开卡顿已作为性能待办进入后续修复队列。
