...
- **固定大纲宽度可调节（ISS-217）**：固定态大纲左栏右缘新增拖拽手柄（样式与右侧 Word 预览 resizer 同构），可拖动调整大纲宽度（200px ~ 容器 40%，上限 480px），双击恢复默认 260px。未固定时轻量横线轨道与展开面板行为不变。

- **「总是固定大纲」开关收敛到设置页（ISS-217，触碰 DESIGN.md 记载的面板内开关设计）**：原先固定态大纲面板头部下方的「总是固定大纲」开关按钮移除，改在「设置 → 外观」分区提供同名开关（新增说明文案，三语言同步）。AppLayout 经 `useSettings` 订阅变更即时生效：开启后当前及后续含标题文档默认固定大纲；在设置页关闭后当前文档大纲随之取消固定。大纲面板内的固定 / 取消固定按钮保留——固定态下点击「取消固定大纲」仍会顺带关闭「总是固定大纲」偏好（否则设置项会把大纲立即钉回）。
...
- **安全加固：fs 插件调用面补齐敏感目录拒绝清单 + 受管资产写入命令强制约束（ISS-197）**：Tauri v2 的 ACL 语义是「allow 列表为空 = 放行一切」（`CommandScope::matches`），此前 capabilities 里的 4 条 `fs:allow-*` 均未配置任何 `fs:scope`——意味着插件层对任意路径的读写不受约束，lib.rs 在自定义命令上做的扩展名白名单 / denied-root 黑名单 / 遍历防护全部可被 `invoke('plugin:fs|read_file', …)` 直接绕过（文档读写实际走自定义命令，插件面仅覆盖另存二次写入、Word 导出、微信导出、HTML 演示资源读取）。治理三步：(1) capabilities 新增 **deny-only** `fs:scope` 对象，拒绝 `/etc` `/private/etc` `/dev` `/private/dev` `$HOME/.ssh` `.aws` `.gnupg` `.kube` 等系统与用户敏感根（deny-only 不改变既有合法读写路径，文档 / 导出流程零影响）；(2) Rust `write_managed_asset` 把注释口径变成强制约束——rel 第一段必须等于 `<文档基础名>.assets`、文件名必须在图片扩展白名单内（png/jpg/jpeg/gif/webp/svg/bmp/avif/ico/tiff/heic/heif）、单资产字节数 ≤ 20MB，另 read/write_opened_document 强制绝对路径；(3) denied-root 黑名单补 `/private/etc`、`/private/dev`（macOS 真实挂载点，canonicalize 后原前缀比较存在旁路空隙）。附带修复：`urlOpenerService` / `fileLocationService` 用「import 是否成功」探测 Tauri 运行时恒真（浏览器下走 opener invoke 抛 unhandled rejection），统一改为 `'__TAURI_INTERNALS__' in window` 口径并补单测。测试：Rust +9（资产段校验 / 扩展名白名单 / 字节上限 / 绝对路径 / private 挂载点）+ 前端 capabilities 契约测试锚定 deny 列表。**真机验证（NOT_VERIFIED，移交用户）**：常规打开 / 另存为 / Word 导出 / 微信导出 / 图片粘贴保存等涉盘流程回归一遍即可。
- **修复多标签页共享图片资产落盘时的跨文档污染，避免图片数据永久丢失（ISS-196）**：图片粘贴 / 拖入后以 `blob:` 临时地址内联在 Markdown 中，保存时由 `persistPendingImageAssets` 把 pending 字节写入当前文档的 `<doc>.assets/` 并替换为相对路径。但该 store 是窗口内所有 tab 共享的单例，且持久化逻辑「遍历全部 pending、全局标记」——tab B 粘贴了图片而先保存 tab A 时：B 的图片字节被写进 A 的 `.assets/` 目录并被全局 `markPersisted`（含 revoke blob URL），B 后续保存时资产已判为「已持久化」跳过落盘，Markdown 中残留 `blob:` 地址 → 重启后死链且字节从未写入正确位置，不可恢复；同窗口其它 tab 正显示的同一张图也会当场裂图。修法三件套：(1) `persistPendingImageAssets` 新增 `content` 参数，只处理**当前文档正文实际引用**的资产；(2) `ImageAsset` 新增 `persistedInto` 记录字节已写盘的文档路径列表——hash 去重使同一图片可被多个未保存文档同时引用，各文档首次保存时都写自己的 `.assets/`（对已写过的路径幂等跳过）；(3) `markPersisted` 不再提前 revoke / 清空 objectUrl（bytes 本就全程驻留内存），保留替换锚点并保住其它 tab 的渲染。测试：持久化服务 14 项单测（新增 3 项 ISS-196 回归：「未引用资产不落盘不标记」「共享 hash 双文档各自写盘」「相对路径快路径幂等」）+ 存量语义更新。**真机验证（NOT_VERIFIED，移交用户）**：双 tab 各粘贴一张不同图片 → 只保存其一 → 再保存另一 tab → 重启确认两张图都正常显示。
